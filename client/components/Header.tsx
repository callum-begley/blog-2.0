...
const Header: React.FC<HeaderProps> = ({ onClick, currentMode }) => {
  ...
  return (
    <header id='header' className= 'relative text-white dark:text-white max-w-full h-40 place-content-center bg-gradient-to-tr from-zinc-900 to-slate-800 z-40 headerGrid'>
        <div className= 'w-screen h-auto place-content-center absolute'>
          <Nav/>
        </div>
        <FallingText
        ...
        {/* Hide back-to-top button on geo page */}
        {!isGeoPage && (
          ...
          ease-in-out hover:scale-110 rounded-full m-4 h-12 w-12">
            <HashLink smooth to="#header" className="line">
              <img src='/down-arrow.png' className="invert rotate-180 w-8 translate-x-1"  alt=''/>
            </HashLink>
          </button>
        )}
    </header>
