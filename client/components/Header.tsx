...
const Header: React.FC<HeaderProps> = ({ onClick, currentMode }) => {
  return (
    <header id='header' className= 'relative text-white dark:text-white max-w-full h-40 place-content-center drop-shadow-xl/50 bg-gradient-to-tr from-zinc-900 to-slate-800 z-40 headerGrid'>
        ...
        <div className= 'z-40 h-60 w-9/12 justify-self-center font-bold lg:text-7xl md:text-5xl sm:text-3xl text-lg absolute top-0 m-0 p-0 text-shadow- text-shadow-sky-300'>
        <FallingText
        ...
        <button className="fixed right-0 bottom-10 p-1 text-5xl bg-zinc-600 hover:bg-zinc-400 transition duration-500 opacity-30 hover:opacity-100
